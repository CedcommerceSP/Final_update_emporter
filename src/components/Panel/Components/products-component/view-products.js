...
class ViewProducts extends Component {
    ...
    getSingleProductData = () => {
        requests.postRequest('connector/product/getProductById',{'source_product_id': this.state.id})
            .then(data => {
                if (data.success) {
                    let temp = this.state;
                    temp.edited_fields = {};
                    ...
                    if ( !isUndefined(data.data['variant_attribute']) ) {
                        temp['product_data']['variant_attribute'] = JSON.parse(JSON.stringify(data.data['variant_attribute']))
                    } else if ( !isUndefined(data.data['variant_attributes']) ) {
                        temp['product_data']['variant_attributes'] = JSON.parse(JSON.stringify(data.data['variant_attributes']))
                    }
                    let draft = htmlToDraft('<React.Fragment>' + data.data.details['long_description'] + '</React.Fragment>');
                    ...
                    if ( !isUndefined(data.data['variants']) && typeof data.data['variants'] === 'object') {
            ...
            <Page title="View Products" primaryAction={{content:'Back', onClick:() => {
                ...
                <Card>
                    <div className="p-5 row">
                        ...
                        {this.state.img.length > 0 && <div className="col-12 mb-5">
                            <span>
