...
class AmazonInstallationForm extends Component {
                        ...
                        <div className="row">
                            {!isUndefined(this.state.schema) && this.state.schema.map((field) => {
                                ...
                                switch(field.type) {
                                    ...
                                    case 'custom_country':
                                        return (
                                            <div className="col-12 pt-2 pb-2" style={{fontSize:'14px'}} key={this.state.schema.indexOf(field)}>
                                                <Dropdown
                                                    options={field.options}
                                                    onChange={(e) => {
                                                        this.handleChange(field.key, e);
                            ...
                            <div className="col-12 text-right mt-3">
                                <Button onClick={() => {
                                    this.onSubmit();
                                }} disabled={this.state.noChange} primary>Submit</Button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    getAppInstallationForm() {
        ...
        requests.getRequest('connector/get/installationForm', {code:params})
            .then(data => {
        ...
        if (this.state.postType === 'external') {
            ...
            for (let i = 0; i < this.state.schema.length; i++) {
                if (this.state.schema[i].value !== '' && this.state.schema[i].type !== 'checkbox') {
                    ...
                    temp[this.state.schema[i].key] = this.state.schema[i].value;
                } else if (this.state.schema[i].required !== 0 && this.state.hide.indexOf(this.state.schema[i].key) === -1) {
                    flag = false;
                }
            }
            ...
            if ( flag ) {
                if ( this.state.page === 'config' ) {
                    requests.postRequest('amazonimporter/request/setAmazonCredentials',data)
                        .then(data => {
                            if (data.success) {
